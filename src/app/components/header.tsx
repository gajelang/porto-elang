...
/** Define a reusable Icon type if your icons are standard React components */
...
export function Header() {
  return (
    <FloatingDockDesktop className="fixed top-4 left-1/2 -translate-x-1/2 z-50" />
  );
}
      ...
      <div className="flex gap-4">
        {navItems.map((item) => (
          <IconContainer
            mouseX={mouseX}
            key={item.name}
            title={item.name} // pass it here for accessibility
            href={item.href}
            icon={item.icon}
          />
        ))}
      ...
      <div className="flex gap-4">
        {contactItems.map((item) => (
          <ContactIcon
            ...
            href={item.href}
            icon={item.icon}
            name={item.name} // pass it here for accessibility
          />
        ))}
      </div>
    </motion.div>
...
/**
 * If you don't actually need `title` in the UI, you can use it at least
 * as an aria-label for accessibility or remove it entirely to avoid
...
function IconContainer({
  ...
  title: string;
  href: string;
  icon: IconType; // More specific than 'any'
}) {
...
function ContactIcon({
  ...
  /** Use name for aria-label to avoid unused-vars warning */
  return (
    <motion.a
      href={href}
      target="_blank"
